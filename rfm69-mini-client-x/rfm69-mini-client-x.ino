...
byte nodeAddr;       // address in (local) RFM69 network
unsigned int nodeID; // nodeID in MQTT topic

Adafruit_BMP280 bmp; // I2C

// RFM69
RF69 rf (10);               // Arduino: CS for RFM69 module
uint8_t RF69_resetpin = 9;  // Adafruit module
...
void sensor0Publish() {
  ...
  putByte(digitalRead(button0));

  putByte(3); // channel 3: button3
  putByte(LPP_DIGITAL_INPUT);
  putByte(digitalRead(button1));
...
void sensor1Publish() {
  ...
  int baro = bmp.readPressure() / 10.0;
  putInt(baro);

//  putByte(6);
//  putByte(LPP_RELATIVE_HUMIDITY);
//  int hum = bmp.readHumidity() * 2;
//  putByte(hum);
...
void setup() {
 ...
 //  UNO: i2c fixed, on pins SCL: A5, SDA: A4
  Serial.begin(9600);
  Serial.println();
  Serial.println("[iot2019-mini-node-bmp-rfm69]");
